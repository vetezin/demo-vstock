...
(function () {
  ...
  function garantirDadosOficina() {
    ...
    if (!Array.isArray(db.oficinaVeiculos)) db.oficinaVeiculos = [];
    if (!db.oficinaVeiculos.length) db.oficinaVeiculos.push({ veiculoId: 1, clienteId: cliente.clienteId, placa: 'ABC1D23', marca: 'Honda', modelo: 'Civic', anoFabricacao: 2022, cor: 'Prata', observacao: 'Veículo de demonstração', clienteNome: cliente.nome, ativo: true });
    if (db.oficinaVeiculos.length < 3) db.oficinaVeiculos.push({ veiculoId: 2, clienteId: cliente.clienteId, placa: 'DEF4G56', marca: 'Toyota', modelo: 'Corolla', anoFabricacao: 2021, cor: 'Branco', observacao: 'Revisão de 40 mil km', clienteNome: cliente.nome, ativo: true }, { veiculoId: 3, clienteId: cliente.clienteId, placa: 'HIJ7K89', marca: 'Volkswagen', modelo: 'T-Cross', anoFabricacao: 2023, cor: 'Cinza', observacao: '', clienteNome: cliente.nome, ativo: true });
    if (!Array.isArray(db.oficinaServicos)) db.oficinaServicos = [];
    if (!db.oficinaServicos.length) db.oficinaServicos.push({ servicoId: 1, categoriaId: categoria.catCod || categoria.catId || 1, descricao: 'Troca de óleo e filtros', detalhes: 'Óleo sintético e filtro de óleo', valorServico: 180, categoriaDescricao: categoria.catDescr || categoria.descricao || 'Serviços gerais', ativo: true });
    if (db.oficinaServicos.length < 3) db.oficinaServicos.push({ servicoId: 2, categoriaId: categoria.catCod || categoria.catId || 1, descricao: 'Alinhamento e balanceamento', detalhes: 'Conferência completa da suspensão', valorServico: 140, categoriaDescricao: categoria.catDescr || 'Serviços gerais', ativo: true }, { servicoId: 3, categoriaId: categoria.catCod || categoria.catId || 1, descricao: 'Diagnóstico eletrônico', detalhes: 'Leitura de módulos e sensores', valorServico: 220, categoriaDescricao: categoria.catDescr || 'Serviços gerais', ativo: true });
    if (!Array.isArray(db.oficinaOrcamentos)) db.oficinaOrcamentos = [];
    if (!db.oficinaOrcamentos.length) db.oficinaOrcamentos.push({ orcamentoId: 1001, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 1, veiculoDescricao: 'Honda Civic • ABC1D23', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, diagnostico: 'Revisão preventiva', observacao: 'Demonstração VStock', valorDesconto: 0, valorTotal: 320, status: 'ABERTO', createdAt: nowIso(), itens: [{ tipo: 'SERVICO', servicoId: 1, descricao: 'Troca de óleo e filtros', quantidade: 1, valor: 180, valorUnitario: 180, subtotal: 180 }, { tipo: 'SERVICO', servicoId: 2, descricao: 'Alinhamento e balanceamento', quantidade: 1, valor: 140, valorUnitario: 140, subtotal: 140 }] });
    if (db.oficinaOrcamentos.length < 3) db.oficinaOrcamentos.push({ orcamentoId: 1002, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 2, veiculoDescricao: 'Toyota Corolla • DEF4G56', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, diagnostico: 'Revisão de suspensão', observacao: 'Aguardando aprovação do cliente', valorDesconto: 20, valorTotal: 260, status: 'APROVADO', createdAt: nowIso(), itens: [{ tipo: 'SERVICO', servicoId: 2, descricao: 'Alinhamento e balanceamento', quantidade: 1, valor: 140, subtotal: 140 }, { tipo: 'SERVICO', servicoId: 3, descricao: 'Diagnóstico eletrônico', quantidade: 1, valor: 140, subtotal: 140 }] }, { orcamentoId: 1003, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 3, veiculoDescricao: 'Volkswagen T-Cross • HIJ7K89', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, diagnostico: 'Troca de pastilhas', observacao: 'Orçamento cancelado para demonstração', valorDesconto: 0, valorTotal: 390, status: 'CANCELADO', createdAt: nowIso(), itens: [{ tipo: 'SERVICO', servicoId: 3, descricao: 'Diagnóstico eletrônico', quantidade: 1, valor: 220, subtotal: 220 }] });
    if (!Array.isArray(db.oficinaOrdensServico)) db.oficinaOrdensServico = [];
    if (!db.oficinaOrdensServico.length) db.oficinaOrdensServico.push({ ordemServicoId: 2001, orcamentoId: 1001, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 1, veiculoDescricao: 'Honda Civic • ABC1D23', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, status: 'EM_ANDAMENTO', diagnostico: 'Revisão preventiva', observacao: 'OS demonstrativa', createdAt: nowIso(), iniciadoEm: nowIso(), dataPrevisaoEntrega: todayIso(), valorDesconto: 0, valorTotal: 180, itens: [{ tipo: 'SERVICO', servicoId: 1, descricao: 'Troca de óleo e filtros', quantidade: 1, valor: 180, valorUnitario: 180, subtotal: 180 }] });
    if (db.oficinaOrdensServico.length < 3) db.oficinaOrdensServico.push({ ordemServicoId: 2002, orcamentoId: 1001, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 2, veiculoDescricao: 'Toyota Corolla • DEF4G56', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, status: 'ABERTA', diagnostico: 'Alinhamento preventivo', observacao: '', createdAt: nowIso(), dataPrevisaoEntrega: todayIso(), valorDesconto: 0, valorTotal: 140, itens: [{ tipo: 'SERVICO', servicoId: 2, descricao: 'Alinhamento e balanceamento', quantidade: 1, valor: 140, subtotal: 140 }] }, { ordemServicoId: 2003, orcamentoId: 1001, clienteId: cliente.clienteId, clienteNome: cliente.nome, veiculoId: 3, veiculoDescricao: 'Volkswagen T-Cross • HIJ7K89', funcionarioId: funcionario.funcCpf, funcionarioNome: funcionario.funcNome, status: 'FINALIZADA', diagnostico: 'Diagnóstico eletrônico', observacao: 'Serviço concluído', createdAt: nowIso(), dataEntrega: todayIso(), valorDesconto: 0, valorTotal: 220, itens: [{ tipo: 'SERVICO', servicoId: 3, descricao: 'Diagnóstico eletrônico', quantidade: 1, valor: 220, subtotal: 220 }] });
    db.oficinaOrcamentos.forEach(o => { o.createdAt ||= o.criadoEm || nowIso(); o.itens = (o.itens || []).map(i => ({ ...i, valor: Number(i.valor ?? i.valorUnitario ?? i.subtotal ?? 0) })); });
    db.oficinaOrdensServico.forEach(o => { o.createdAt ||= o.criadoEm || nowIso(); o.dataPrevisaoEntrega ||= todayIso(); o.itens = (o.itens || []).map(i => ({ ...i, valor: Number(i.valor ?? i.valorUnitario ?? i.subtotal ?? 0) })); });
    persistDb();
  }
  ...
  function oficinaCatalogo(url) {
    const produtos = buildEstoqueResumo().map(p => ({ tipo: 'PRODUTO', produtoCod: p.prod_cod, descricao: p.prod_descr, valor: Number(p.valor_unitario || 0), valorUnitario: Number(p.valor_unitario || 0), estoqueDisponivel: Number(p.saldo || 0), categoriaId: null, categoriaDescricao: 'Produtos', ativo: p.ativo !== false }));
    const servicos = db.oficinaServicos.filter(s => s.ativo !== false).map(s => ({ tipo: 'SERVICO', servicoId: s.servicoId, descricao: s.descricao, detalhes: s.detalhes || '', valor: Number(s.valorServico || 0), valorUnitario: Number(s.valorServico || 0), categoriaId: s.categoriaId, categoriaDescricao: s.categoriaDescricao || 'Serviços' }));
  ...
  async function handleApi(url, init) {
    ...
    const veiculoMatch = path.match(/^\/api\/oficina\/veiculos\/(\d+)(?:\/status)?$/); if (veiculoMatch && method === 'PATCH') { const v = db.oficinaVeiculos.find(x => Number(x.veiculoId) === Number(veiculoMatch[1])); if (!v) return text('Veículo não encontrado.', 404); v.ativo = url.searchParams.has('ativo') ? url.searchParams.get('ativo') === 'true' : parseJsonBody(init).ativo !== false; persistDb(); return json(v); }
    ...
    const servicoMatch = path.match(/^\/api\/oficina\/servicos\/(\d+)(?:\/status)?$/); if (servicoMatch && method === 'PATCH') { const s = db.oficinaServicos.find(x => Number(x.servicoId) === Number(servicoMatch[1])); if (!s) return text('Serviço não encontrado.', 404); s.ativo = url.searchParams.has('ativo') ? url.searchParams.get('ativo') === 'true' : parseJsonBody(init).ativo !== false; persistDb(); return json(s); }
